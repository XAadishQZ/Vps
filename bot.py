"""
EagleNode Host - Discord Bot (Fixed Version)
Fully working version with correct VPS creation.
"""
...
@tree.command(name="create", description="Create a new VPS")
@app_commands.describe(name="VPS name", image="Docker image", memory="Memory limit", cpus="CPU limit")
async def create_cmd(interaction: discord.Interaction, name: str, image: Optional[str] = DEFAULT_OS_IMAGE,
    ...
    try:
        ...
        save_backup()
        await interaction.followup.send(f"✅ VPS `{cname}` created (image: `{image}`).", ephemeral=False)
    except Exception as e:
